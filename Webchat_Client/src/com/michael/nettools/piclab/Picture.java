    ...
    public static Picture fromText(String text){
        ...
        for (int i = 1, pixelsLength = pixels.length; i < pixelsLength; i++) {
            ...
            for (int j = 1; j < row.length; j++) {
                String pixel = row[j];
                String[] colors = pixel.split(",");
                Pixel curPixel = new Pixel(out, j, i);

                if (colors.length == 3) {
                    System.out.println(j +", " + i + ": "+ colors.length);
                    curPixel.setRed(Integer.parseInt(colors[0]));
                    curPixel.setGreen(Integer.parseInt(colors[1]));
                    curPixel.setBlue(Integer.parseInt(colors[2]));
